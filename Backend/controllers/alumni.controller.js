...
const addAlumni = async (req, res) => {
  try {
    ...
    const existingAlumni = await Alumni.findOne({
      "paymentInfo.transactionId": data.paymentInfo.transactionId,
    });
    if (existingAlumni) {
      return res
        .status(400)
    ...
    if (req.file) {
      ...
      const filename = `${data.personalInfo.roll}_${Date.now()}-${
    ...
    if (req.file) {
      ...
      fs.writeFile(filePath, req.file.buffer, async (err) => {
        if (err) {
          return res
            .status(500)
            .json({ success: false, message: "Error saving new image" });
        }
  ...
  try {
    ...
    if (!validStatuses.includes(Number(status))) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid status value. Use 0 for Not Paid, 1 for Paid, or 2 for Rejected.",
      });
    }
    ...
    const statusMessage =
      status === "1" ? "Paid" : status === "0" ? "Not Paid" : "Rejected";
  ...
  try {
    ...
    if (!alumni) {
      return res
        ...
        .json({
          success: false,
          message: "Alumni not found or transaction ID mismatch",
        });
    }
